        ...
        for read in reads:
            ...
            if m:
                ...
                if not os.path.exists('./%s/%s' %(rep_read_dir, outdir)):
                    relocaTE = '%s --te_fasta %s --genome_fasta %s --fq_dir %s --outdir %s --reference_ins %s --step 34567 --len_cut_match 10 --len_cut_trim 10 --mismatch 2 --aligner blat' %(RelocaTE, Repeat, Reference, read_dir, outdir, existingTE)
