    ...
    for rep in reps:
        ...
        for call in calls: 
            ...
            if args.call == 'RelocaTE':
                if 1: #use only inseretions with two breakpoint
                    #MSU7.Chr4.ALL.rep1_reads_10X_100_500_RelocaTE/mPing/results/HEG4.mPing.all_inserts.gff 
                    file_list = glob.glob('%s/*/results/*.all_inserts.gff' %(call))
                    os.system('cat %s > %s/HEG4.%s.all_inserts.gff' %(' '.join(file_list), call, repeat))
                    result_gff = '%s/HEG4.%s.all_inserts.gff' %(call, repeat)
                    #print result_gff
                    ...
                    call_name  = os.path.split(call)[-1]
                    #non_ref_gff= 'grep "Non-reference" %s > %s.temp.relocate.gff' %(result_gff, call_name)
                    #win_overlap= 'bedtools window -w 100 -a %s -b %s.temp.relocate.gff > %s.temp.relocate.overlap' %(ref_gff, call_name, call_name)
                    #print win_overlap
                    ...
                    #call_te = parse_ref_gff('%s.temp.relocate.gff' %(call_name))
                    #eval_list = parse_overlap_reloacte('%s.temp.relocate.overlap' %(call_name), ref_te, call_te)
                    ...
                    file_list = glob.glob('%s/*/results/*.all_nonref.txt' %(call))
                    os.system('cat %s > %s/HEG4.%s.all_nonref.txt' %(' '.join(file_list), call, repeat))
                    non_ref_txt = '%s/HEG4.%s.all_nonref.txt' %(call, repeat)
                    non_ref_gff = '%s/HEG4.%s.all_nonref.gff' %(call, repeat)
                    #print 'cat %s > %s/HEG4.%s.all_nonref.txt' %(' '.join(file_list), call, repeat)
