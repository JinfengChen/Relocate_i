    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2: 
                ...
                tsd_s  = int(unit[3]) - len(tsd_wd) # for ITIS only because the output have one more base
                tsd_e  = int(unit[3])
...
def main():
    ...
    for rep in reps:
        ...
        for call in calls: 
            ...
            if args.call == 'RelocaTE':
                ...
                #awk '{print $1"\tHEG4\tITIS\t"$2"\t"$3"\t.\t"$6"\t.\tID="$1"_"$2";"$4}'
                #MSU7.Chr3.mPing.rep1_reads_9X_100_500_ITIS/MSU7.Chr3.mPing.rep1_reads_9X_100_500.mPing.filtered.bed 
                gff_pre = '_'.join([unit[0], unit[1], unit[2], unit[3], unit[4]])
                #result_bed = '%s/%s.mPing.filtered.bed' %(call, gff_pre)
                #result_gff = '%s/%s.mPing.filtered.gff' %(call, gff_pre)
                result_bed  = '%s/all.filtered.bed' %(call)
                result_gff  = '%s/all.filtered.gff' %(call)
                if not os.path.isfile(result_bed):
                    os.system('cat %s/*.filtered.bed > %s/all.filtered.bed' %(call, call))
                try:
                    os.path.getsize(result_bed) and os.path.getsize(result_bed) > 0
                except:
                    print 'Run not finished: %s\n%s' %(result_bed, result_gff)
                    continue
                bed2gff    = 'awk \'{print $1"\\tHEG4\\tITIS\\t"$2"\\t"$3"\\t.\\t"$6"\\t.\\tID="$1"_"$2";"$4}\' %s > %s' %(result_bed, result_gff)
                #print bed2gff
                os.system(bed2gff)
                win_overlap= 'bedtools window -w 100 -a %s -b %s > temp.itis.overlap' %(ref_gff, result_gff)
                os.system(win_overlap)
                call_te = parse_ref_gff(result_gff)
                eval_list = [np.nan, np.nan, np.nan, np.nan, np.nan]
                if os.path.isfile('temp.itis.overlap') and os.path.getsize('temp.itis.overlap') > 0:
                    eval_list = parse_overlap_reloacte('temp.itis.overlap', ref_te, call_te)
                index = '_'.join([depth, genome, chro, repeat, readlen, libsize, args.call])
                data[index][replicate] = eval_list
                #print index, replicate
                #print eval_list
                os.system('rm temp.itis.overlap')
    ...
    for idx in sorted(data.keys()):
        ...
        call_avg = np.nanmean([eval1[0], eval2[0], eval3[0]])
        call_sd  = np.nanstd([eval1[0], eval2[0], eval3[0]])
        true_avg = np.nanmean([eval1[1], eval2[1], eval3[1]])
        true_sd  = np.nanstd([eval1[1], eval2[1], eval3[1]])
        tsd_avg  = np.nanmean([eval1[2], eval2[2], eval3[2]])
        tsd_sd   = np.nanstd([eval1[2], eval2[2], eval3[2]])
        Non_avg  = np.nanmean([eval1[3], eval2[3], eval3[3]])
        Non_sd   = np.nanstd([eval1[3], eval2[3], eval3[3]])
        False_avg  = np.nanmean([eval1[4], eval2[4], eval3[4]])
        False_sd   = np.nanstd([eval1[4], eval2[4], eval3[4]])
